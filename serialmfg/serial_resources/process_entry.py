...
class ProcessEntry:
    ...
    def add_number(self, dataset_name, value, usl=None, lsl=None):
        ...
        try:
            ...
            if serial.allow_automatic_dataset_creation:
                dataset = serial.Datasets.create(dataset_name, "NUMERICAL", self.process_id)
            else:
                raise e
    ...
    def _upload_file(self, dataset_name, path, file_name, dataset_type):
        ...
        dataset = None
        try:
            dataset = serial.Datasets.get(dataset_name, data_type, self.process_id)
        except Exception as e:
            if serial.allow_automatic_dataset_creation:
                dataset = serial.Datasets.create(dataset_name, data_type, self.process_id)
            else:
                raise e
